...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    ...
    if (
      new Date(`${day}T${time}`) < new Date(`${day}T${restaurant.open_time}`) || new Date(`${day}T${time}`) > new Date(`${day}T${restaurant.close_time}`)
    ) {
      return res.status(400).json({
        errorMessage: "Restaurant is not opne at the that time"
      })
    }
    ...
    if (!searchTimeWithTables) {
      return res.status(400).json({
        errorMessage: "No availabity, cannot book"
      })
    }
    ...
    const booking = await prisma.booking.create({
      data: {
        number_of_peple: parseInt(partySize),
